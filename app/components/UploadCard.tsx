  ...
  return (
    <div className="w-full max-w-xl mx-auto z-10">
      ...
      <motion.div
        ...
        className={`relative overflow-hidden rounded-3xl p-10 flex flex-col items-center justify-center border-2 border-dashed transition-all duration-300 ${
          isBackendConnected === false
            ? "border-slate-200 bg-slate-50 cursor-not-allowed opacity-60"
            : isDragActive
            ? "border-teal-500 bg-teal-50/40 shadow-lg shadow-teal-500/5 cursor-pointer"
            : "border-slate-200 bg-white hover:border-teal-400 hover:bg-slate-50/50 hover:shadow-md cursor-pointer"
        }`}
      ...
      >
        ...
        <div className="flex flex-col items-center text-center space-y-6">
          <motion.div
            className={`p-4.5 rounded-2xl ${
              isDragActive ? "bg-teal-100 text-teal-600" : "bg-slate-100 text-slate-500"
            }`}
            animate={{
              y: isDragActive ? -6 : 0,
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-slate-500 max-w-sm">
              ...
              <span className="text-teal-600 font-semibold hover:underline">browse files</span>.
            </p>
          </div>

          <div className="flex items-center space-x-2 text-[11px] text-slate-400 bg-slate-50 px-3.5 py-1.5 rounded-full border border-slate-100">
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="mt-4 p-4 rounded-2xl bg-rose-50 border border-rose-100 flex items-start space-x-3 text-rose-800 text-xs"
          >
